...
function isValid(board,bW,bB,pl,src,dest,die){const bar=pl===WHITE?bW:bB;if(src===BAR&&bar===0)return false;if(src!==BAR&&bar>0)return false;if(src!==BAR){if(pl===WHITE&&board[src]<=0)return false;if(pl===BLACK&&board[src]>=0)return false}if(dest===OFF){if(!canBearOff(board,pl))return false;if(src!==BAR){const ed=pl===WHITE?src-die:src+die;if(ed<0||ed>23){if(pl===WHITE){for(let i=src+1;i<=5;i++)if(board[i]>0)return false}else{for(let i=src-1;i>=18;i--)if(board[i]<0)return false}}}return true}if(pl===WHITE&&board[dest]<-1)return false;if(pl===BLACK&&board[dest]>1)return false;return true}
